...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-4.13-x86_64 -g /boot/archiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_boot() {
    ...
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-4.13-x86_64 ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz
}
...
make_syslinux() {
    ...
    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/*-MANJARO/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
